...
def render_main_tabs(df, df_filtered, params):
    ...
    with tab_vent:
        ...
        if "wind10_dir" in df_filtered.columns and "wind10_spd_kt" in df_filtered.columns:
            _c10 = df_filtered["wind10_dir"].apply(
                lambda d: _dirs[round(float(d)/22.5)%16] if pd.notna(d) and str(d) not in ["","None","nan"] else "")
            for x, y, c in zip(_x, df_filtered["wind10_spd_kt"].tolist(), _c10.tolist()):
                if c and pd.notna(y):
        ...
        if "wind100_dir" in df_filtered.columns and "wind100_spd_kt" in df_filtered.columns:
            ...
            _c100 = df_filtered["wind100_dir"].apply(
                lambda d: _dirs[round(float(d)/22.5)%16] if pd.notna(d) and str(d) not in ["","None","nan"] else "")
            for x, y, c in zip(_x, df_filtered["wind100_spd_kt"].tolist(), _c100.tolist()):
                if c and pd.notna(y):
                    fig_wind.add_annotation(x=x, y=y, text=f"<b>{c}</b>",
                        showarrow=False, font=dict(size=8, color="#40c057"), yshift=12, row=row100, col=1)
...
def main():
    ...
    # ── Chargement données (avant sidebar pour alimenter filtre période)
    is_demo   = False
    is_github = False
    csv_name  = ""
    ...
    st.session_state["df_loaded"] = df   # pour le filtre période dans sidebar
